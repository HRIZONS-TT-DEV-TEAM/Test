const cds = require('@sap/cds/lib'), { fs, path, tar, rimraf, read, readdir } = cds.utils
...
const { mkdirp, write } = require('@sap/cds/lib/utils')
...
const _includeExt = () => {
  return (cds.env.requires['cds.xt.ExtensibilityService']?.['check-existing-extensions'] === true)
}
class DbAdapter {
  ...
  async init() {
    ...
    cds.on('serving:cds.xt.JobsService', js => {
      js.on('taskUpdate', async (msg) => {
        if (msg.data?.task?.service === 'cds.xt.ExtensibilityService') { // Only filter for service. There is only one operation that triggers jobs
          await this.emit('activated', { tenant: msg.tenant, job_ID: msg.data.task.job_ID, status: msg.data.task.status, error: msg.data.task.error })
        }
      })
    })
    ...
    this.on('activate', async req => {
      const { ID: tag, level: status = ACTIVE, options } = req.data
      const tenant = _tenant(req)
      if (tenant) cds.context = { tenant }
    ...
    this.on('READ', 'Extensions', async req => {
      const tenant = _tenant(req) // TODO add to compat later
      if (tenant) cds.context = { tenant }
