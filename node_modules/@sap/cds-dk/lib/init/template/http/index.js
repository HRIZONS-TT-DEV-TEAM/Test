...
module.exports = class HttpTemplate extends require('../../plugin') {
  ...
  async #generateRequests(services, payload, csn) {
    ...
    for (const [serviceName, service] of services) {
      let [hostname, auth] = await this.#getHostnameAndAuth(service, csn)
      let requests = []
  ...
  async #getHostnameAndAuth(service, csn) {
    if (!cds.cli.options['for-app']) {
      ...
      const users = Object.values(await this.#mockUsersForService(service, csn))
      const authStr = users.length ? `Authorization: Basic ${users[0].name}:${users[0].password||''}` : null
      return [host, authStr]
    }
   ...
   */
  async #mockUsersForService(service, csn) {
    const declaredRoles = new Set()
    Object.values(csn.definitions)
      .filter(d => d.name.startsWith(service.name))
    ...
    if (declaredRoles.size && !matchingUsers.length) {
